			...
			for (int j = 0; j < N_GRAPHS; j++)
			{
				...
				if (FST::execute(fstex))
				{
					...
					case LEX_ID:
					{
						...
						if (Tables.Lextable.table[i - 2].lexema == LEX_CREATE && IT::IsId(Tables.IDtable, InStruct.tokens[i].token) != -1)
						{
						...
						if (IT::IsId(Tables.IDtable, InStruct.tokens[i].token) == -1)
						{
							...
							if (Tables.Lextable.table[i - 1].lexema == LEX_FUNCTION)
							{
								...
								IT::Entry entryit(tempTokenFunction, i, IT::NUM, IT::F);
