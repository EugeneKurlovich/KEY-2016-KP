...
namespace GRB
{
	Greibach greibach(NS('S'), TS('$'), 6,
		Rule(NS('S'), GRB_ERROR_SERIES + 15, 3,   //  —труктура программы
		...
		Rule(NS('F'), GRB_ERROR_SERIES + 16, 2,
			Rule::Chain(2, TS('n'), TS('i')),   // параметры ф-ии
			Rule::Chain(4, TS('n'), TS('i'), TS(','), NS('F'))

		),
		Rule(NS('N'), GRB_ERROR_SERIES + 17, 8,  // возможные конструкции в ф-и€х
		...
		Rule(NS('E'), GRB_ERROR_SERIES + 18, 15, // выражени€
			Rule::Chain(1, TS('i')),
		...
		Rule(NS('W'), GRB_ERROR_SERIES + 19, 6, // принимаемые параметры ф-ии
			Rule::Chain(2, TS('-'), TS('l')),
		...
		Rule(NS('M'), GRB_ERROR_SERIES + 20, 8, // знаки
