...
bool  SemanticAnalyze(LT::LexTable &Lextable, In::IN &InStruct, IT::IdTable &idtable, Log::LOG &log)
{
	...
	for (int i = 0; i < Lextable.size; i++)
	{
		...
		/*if (Lextable.table[i-1].lexema == LEX_EQUAL && Lextable.table[i].lexema == LEX_ID && Lextable.table[i + 1].lexema == LEX_LEFTHESIS)
					...
					if (IT::IsId(idtable, InStruct.tokens[u].token) != -1)
						{
							...
							if (idtable.table[checkparms].iddatatype == IT::NUM)
								k1[size1++] = 1;
							else
								k1[size1++] = 0;
						}
				...
				while (Lextable.table[gg].lexema != LEX_RIGHTTHESIS)
				{
					...
					if (Lextable.table[gg].lexema == LEX_ID || Lextable.table[gg].lexema == LEX_LITERAL)
					{
						if (IT::IsId(idtable, InStruct.tokens[gg].token) != -1)
						{
							...
							if (idtable.table[checkin].iddatatype == IT::NUM)
								k2[size2++] = 1;
							else
								k2[size2++] = 0;
						}
			...
			if (IT::IsId(idtable, InStruct.tokens[i].token) != -1 &&  (Lextable.table[i-2].lexema == LEX_KOREN || Lextable.table[i - 2].lexema == LEX_STEP))
			{
				int check;
				check = IT::IsId(idtable, InStruct.tokens[i].token);
				if (idtable.table[check].iddatatype == IT::WRD && (Lextable.table[i-2].lexema == LEX_KOREN || Lextable.table[i - 2].lexema == LEX_STEP))
				{
			...
			if (IT::IsId(idtable, InStruct.tokens[i].token) !=-1 && idtable.table[IT::IsId(idtable, InStruct.tokens[i].token)].iddatatype == IT::NUM && InStruct.tokens[i+2].token[0] == char(39))
			{
				Log::writeError(log.stream, Error::GetError(11, Lextable.table[i].sn, NULL));
				choise = false;
			}

			if (IT::IsId(idtable, InStruct.tokens[i].token) != -1 && idtable.table[IT::IsId(idtable, InStruct.tokens[i].token)].iddatatype == IT::WRD&& Lextable.table[i-1].lexema == LEX_SEPARATOR && InStruct.tokens[i + 2].token[0] != char(39))
			{
				Log::writeError(log.stream, Error::GetError(11, Lextable.table[i].sn, NULL));
				choise = false;
			}
